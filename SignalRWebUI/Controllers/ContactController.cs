	...
	public class ContactController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			if (responseMessage.IsSuccessStatusCode)
			{
				...
				var values = JsonConvert.DeserializeObject<List<ResultContactDto>>(jsonData); //list = deserialize,  add & update = serialize
				return View(values);
			}
			return View();
		}
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7063/api/Contact/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"https://localhost:7063/api/Contact/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		[HttpPost]
		public async Task<IActionResult> UpdateAbout(UpdateContactDto updateContactDto)
		{
			var client = _httpClientFactory.CreateClient();
